# nohup python web_interface_file.py > log/20240115.log 2>&1 &
...
parser.add_argument('--label_file_dir', default='data/cancer.xlsx')
parser.add_argument('--model_result_name', default='our_test_label')
args = parser.parse_args()

label_file_dir = args.label_file_dir
...
'BIO-BBU_predict_label','BBU_predict_label','svm','new_final_result',
...
def combine_evaluate(predict_label_file):
    ...
    for model_label_name in model_label_names:
        labels_all = list(all_label_df['final_result'])
        predlabels_all = list(all_label_df[model_label_name])
        predscores_all = list(all_label_df[model_label_name.replace('label','score')])
        ml_compare = rpf.ML_compare(model_label_name,labels_all, predlabels_all,predscores_all,
...
# model_configs = get_folder_dir(model_configs_dir)         
# for model_config in model_configs:
    # run_bert(model_config,test_file_dir)
# write_predict_label()
